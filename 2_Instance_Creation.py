...
# vincenty在geopy2.0中被移除了，文档建议改用geodesic
...
def compute_trip_motion_features(all_trip_one_user, data_type):
    ...
    :param all_trip_one_user: a sequence of GPS points
    ...
    """
    # trip结构[point][纬度，经度，时间] or [point][纬度，经度，时间，交通模式]
    all_trip_motion_features_one_user = []
    for trip in all_trip_one_user:
        # 遍历每个用户的所有trip
        # 因为要计算加加速度，一个trip至少4个点
        if len(trip) >= 4:
            relative_distance = []
            delta_time = []
            relative_speed = []
...
def trip_check_thresholds(trip_motion_all_user, min_threshold, min_distance, min_time, data_type):
    ...
    if data_type == 'labeled':
        for user in trip_motion_all_user:
            all_user.append(
                list(filter(lambda trip: len(trip[0][0]) >= min_threshold and np.sum(trip[0][0, :]) >= min_distance
