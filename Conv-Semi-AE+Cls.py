# SECA代码
# 将原来的tensorflow 1适配了tensorflow 2，并做了debug
import numpy as np
...
def encoder_network(latent_dim, num_filter_ae_cls, input_combined, input_labeled):
    ...
    layers_shape = []
    # 注：如果输入为空的话，就会报input and filter must have the same depth
    for i in range(len(num_filter_ae_cls)):
        ...
        with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE, initializer=initializer):
            encoded_combined = tf.compat.v1.layers.conv2d(inputs=encoded_combined, activation=tf.nn.relu,
                                                          filters=num_filter_ae_cls[i],
        ...
        with tf.compat.v1.variable_scope(scope_name, reuse=True, initializer=initializer):
            encoded_labeled = tf.compat.v1.layers.conv2d(inputs=encoded_labeled, activation=tf.nn.relu,
                                                         filters=num_filter_ae_cls[i],
                                                         name='conv_1', kernel_size=kernel_size, strides=strides,
                                                         padding=padding)
...
def decoder_network(latent_combined, input_size, kernel_size, padding, activation):
    ...
    if len(num_filter_) % 2 == 0:
        ...
        for i in range(len(num_filter_)):
            ...
            with tf.compat.v1.variable_scope(scope_name, initializer=initializer):
                filter_size, activation = (input_size[-1], tf.nn.sigmoid) if i == len(num_filter_) - 1 else (
                    int(num_filter_[i] / 2), tf.nn.relu)
...
def classifier_mlp(latent_labeled, num_class, num_filter_cls, num_dense):
    ...
    for i in range(len(num_filter_cls)):
        ...
        if len(num_filter_cls) % 2 == 0:
            if i % 2 != 0:
                conv_layer = tf.compat.v1.layers.max_pooling2d(conv_layer, pool_size=pool_size, strides=pool_size,
                                                               name='pool')
        else:
            if i % 2 == 0:
                conv_layer = tf.compat.v1.layers.max_pooling2d(conv_layer, pool_size=pool_size, strides=pool_size,
...
def semi_supervised(input_labeled, input_combined, true_label, alpha, beta, num_class, latent_dim, num_filter_ae_cls,
                    num_filter_cls, num_dense, input_size):
    latent_combined, latent_labeled, layers_shape = encoder_network(latent_dim=latent_dim,
                                                                    num_filter_ae_cls=num_filter_ae_cls,
    ...
    classifier_output, dense = classifier_mlp(latent_labeled, num_class, num_filter_cls=num_filter_cls,
                                              num_dense=num_dense)
    ...
    loss_cls = tf.reduce_mean(
        input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=true_label, logits=classifier_output),
        name='loss_cls')
    total_loss = alpha * loss_ae + beta * loss_cls
    # total_loss = beta * loss_ae + alpha * loss_cls
    loss_reg = tf.reduce_sum(
        input_tensor=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES, 'EasyNet'))
...
def training(one_fold, X_unlabeled, seed, prop, num_filter_ae_cls_all, epochs_ae_cls=epoch):
    ...
    for z in range(len(num_filter_ae_cls_all)):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            input_combined = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None] + input_size,
                                                      name='input_combined')
            ...
            for k in range(epochs_ae_cls):
                ...
                if len(X_ae) >= 1 and len(X_cls) >= 1 and len(Y_cls) >= 1:
                    loss_ae_, loss_cls_, accuracy_cls_, _ = sess.run([loss_ae, loss_cls, accuracy_cls, train_op],
                                                                     feed_dict={alpha: alfa_val, beta: beta_val,
                                                                                ...
                                                                                true_label: Y_cls})
                    print('Epoch Num {}, Batches Num {}, Loss_AE {}, Loss_cls {}, Accuracy_train {}'.format
                          (k, i + 1, np.round(loss_ae_, 3), np.round(loss_cls_, 3), np.round(accuracy_cls_, 3)))

                print('====================================================')
                loss_val, acc_val = loss_acc_evaluation(Val_X, Val_Y, loss_cls, accuracy_cls, input_labeled, true_label,
                                                        k, sess)
                val_loss.update({k: loss_val})
                val_accuracy.update({k: acc_val})
                print('====================================================')
                try:
                    os.makedirs("./Conv-Semi-AE+CLS/" + str(z) + '/')
                except:
                    pass
                saver.save(sess, "./Conv-Semi-AE+CLS/" + str(z) + '/' + str(prop), global_step=k)
                ...
                if all([change_to_ae, val_accuracy[k] < val_accuracy[k - 1], val_accuracy[k] < val_accuracy[k - 2]]):
                    ...
                    save_path = "./Conv-Semi-AE+CLS/" + str(z) + '/' + str(prop) + '-' + str(max_acc)
                    ...
                    # #checkpoint = os.path.join(os.getcwd(), save_path)
                    max_acc = max(val_accuracy.items(), key=lambda k: k[1])[0]
                    # saver.restore(sess, "/Conv-Semi-AE+CLS/" + str(prop) + '/' + str(max_acc) + ".ckpt")
                    save_path = "/Conv-Semi-AE+CLS/" + '2/' + str(z) + '/' + str(prop) + '-' + str(max_acc)
                    saver.restore(sess, save_path)
